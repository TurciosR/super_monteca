    ...
    if (event.keyCode == 13) {
      ...
      if ($(this).val() != "") {
        ...
        $.ajax({
          type: 'POST',
          url: "venta.php",
          data: "process=cargar_data&n_ref=" + n_ref + "&fecha=" + fecha+ "&caja=" + caja,
          dataType: 'json',
...
function senddata() {
  ...
  ubicacion_venta=$('#ubicacion_venta').val();
  ...
  dataString += '&ubicacion_venta='+ubicacion_venta;
...
function addProductList(id_proda, tip)
{
  ...
  urlprocess = "venta.php";
  var dataString = 'process=consultar_stock'+'&id_producto='+id_proda+'&id_factura='+id_factura+'&tipo='+tip+'&precio_aut='+precio_aut+'&ubicacion_venta='+ubicacion_venta;
  $.ajax({
    type: "POST",
    url: urlprocess,
    data: dataString,
    ...
    success: function(data)
        ...
        id_producto = id_prod
        existencia = parseFloat(existencias);
        existencia = round(existencia, 4);
        a_cant = 1;
        unidad = parseInt(unidades);
        a_cant = parseFloat(a_cant * unidad);
        a_asignar = 0;
        $('#inventable tr').each(function(index) {
          if ($(this).find('.id_pps').text() == id_producto) {
        ...
        filas++;
	      ...
	      var cantidades = "<td class='cell100 column10 text-success'><input type='text'  class='form-control decimal2  " + categoria+ " cant' id='cant' name='cant' value='"+a_cant+"' style='width:60px;'></td>";
	      tr_add = '';
