	...
	array( 'db' => '`fac`.`id_factura`', 'dt' => 7, 'formatter' => function( $id_factura, $row ){
		...
		array( 'db' => '`fac`.`id_factura`', 'dt' => 8, 'formatter' => function( $id_factura, $row ){
			$menudrop="<div class='btn-group'>
				...
				$id_sucursal=$_SESSION['id_sucursal'];
				$admin=$_SESSION["admin"];
				$sql="SELECT numero_doc,finalizada,anulada,tipo,id_apertura,credito FROM factura WHERE id_factura='$id_factura'";
				$result=_query($sql);
				$count=_num_rows($result);
				$row=_fetch_array($result);
				$anulada=$row['anulada'];
				...
				if ($cuenta>0) {
					...
					$sql_aperturaa = _query("SELECT * FROM apertura_caja WHERE vigente = 1 AND id_sucursal = '$id_sucursal' AND id_empleado = '$id_user'");
					$cuenta = _num_rows($sql_aperturaa);
					$turno_vigente=0;
					if ($cuenta>0) {
						$turno_vigente = 1;
					}
